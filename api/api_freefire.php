...
if(@$_POST['user_id'] != NULL){
    ...
    $random_1 = random('5');
    $waktu = waktu();
    $query = "voucherPricePoint.id=270285&voucherPricePoint.price=20000.0&voucherPricePoint.variablePrice=0&n=$waktu&email=din0812$random_1%40store.com&userVariablePrice=0&order.data.profile=eyJuYW1lIjoiICIsImRhdGVvZmJpcnRoIjoiIiwiaWRfbm8iOiIifQ==&user.userId=$target_id&user.zoneId=&msisdn=0213829389&voucherTypeName=FREEFIRE&shopLang=id_ID&checkoutId=145f627a-125a-42a0-8011-ae1003cc9001&affiliateTrackingId=&impactClickId=&anonymousId=0070f667-d075-4559-b6e5-77090a6c6453&fullUrl=https%3A%2F%2Fwww.codashop.com%2Fid-id%2Ffree-fire&userSessionId=YWxkaW5vd2lsZGhhbjFAZ21haWwuY29t&userEmailConsent=true&userMobileConsent=false&verifiedMsisdn=&promoId=&promoCode=&clevertapId=42c1f6c00d2345c1be0ad69f6b285cfb";
    $link = 'https://order-sg.codashop.com/initPayment.action';
    $curl = curl($link, $headers, $query);
    if(preg_match('/(role|invalid)/i',$curl)){
        //cek error
        if(preg_match('/role/',$curl)){
            $js = json_decode($curl,true);
            $character = $js['confirmationFields']['roles'];
            $nickff = $character[0]['role'];
            if($nickff != NULL){
                $json = array(
                                "status" => "ok",
                                "nama" => urldecode($nickff),
                                "id" => $target_id
                               );
            }else{
